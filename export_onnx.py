...
MODEL_TYPE = "yolo"
REPO_ID = "HyunchanAn/Dental_005"
PT_PATH = "weights/best.pt"  # Update path if different


def main():
    if not os.path.exists(PT_PATH):
        print(f"[Dental_005] No .pt file found at {PT_PATH}.")
        return

    print(f"[Dental_005] Exporting {PT_PATH} to ONNX...")
    ...
    if onnx_path and os.path.exists(onnx_path):
        print(f"[Dental_005] Uploading to HuggingFace {REPO_ID}...")
        api = HfApi()
        # Ensure you are logged in via `huggingface-cli login`
        try:
            api.upload_file(
                path_or_fileobj=onnx_path, path_in_repo="weights/best.onnx", repo_id=REPO_ID, repo_type="model"
            )
            print(f"[Dental_005] Upload successful. Deleting local weights to save space...")
            os.remove(PT_PATH)
            os.remove(onnx_path)
            print(f"[Dental_005] Cleaned up local disk.")
        except Exception as e:
            print(f"[Dental_005] Upload failed: {e}")
