...
repo_dir = r"E:\Github\Caries_Detection_from_Photo"
docs_dir = os.path.join(repo_dir, "docs")
os.makedirs(docs_dir, exist_ok=True)

md_path = os.path.join(docs_dir, "Phase_Field_Enhancement.md")
md_content = """# Phase-Field 기반 Image Segmentation을 활용한 정확도 향상 기획
...
if "## 향후 연구" not in readme_content:
    readme_content += "\n## 향후 연구 (Future Work)\n- [Phase-Field 기반 분할 향상 및 안정성 연구](docs/Phase_Field_Enhancement.md)\n"
    with open(readme_path, "w", encoding="utf-8") as f:
        f.write(readme_content)

log_path = os.path.join(repo_dir, "development_log.txt")
try:
    with open(log_path, "a", encoding="utf-8") as f:
         f.write(f"\n{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n- 이슈 #2 해결: Phase-Field 기반 성능 향상 연구 방향성 기획 문서 작성\n")
except Exception as e:
    pass

subprocess.run(["gh", "issue", "close", "2", "-m", "Phase-Field 관련 연구 내용을 docs/Phase_Field_Enhancement.md 문서에 정리하여 반영 완료했습니다."], cwd=repo_dir)

subprocess.run(["git", "add", "."], cwd=repo_dir)
subprocess.run(["git", "commit", "-m", "docs: Phase-Field 기획 문서화 및 #2 이슈 해결"], cwd=repo_dir)
